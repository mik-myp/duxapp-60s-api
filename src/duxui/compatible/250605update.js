...
file.fileList('src', '.jsx,.tsx,.js,.ts', (filePath) => {
  ...
  // 如果没有新增导入，直接写文件并结束
  if (
    Object.keys(importsToAddMap).length === 0 &&
  ...
  for (const [moduleName, newImportsSet] of Object.entries(importsToAddMap)) {
    ...
    // 新导入项排除已有的
    const newImports = [...newImportsSet].filter(
      (i) => !block.specifiers.includes(i)
    );
    ...
    // 替换内容
    content =
      content.slice(0, block.start) + newImportText + content.slice(block.end);
    ...
    importBlocks[blockIndex].end =
      importBlocks[blockIndex].start + newImportText.length;
