...
export const draw = (context, children = context.svgs, option) => {
  ...
  Children.forEach(children, (child) => {
    ...
    const currentContext = {
      ...context,
      ...(props.clipPath ? createOffCanvas(layout.width, layout.height) : {})
    };
      ...
      switch (key) {
        ...
        case 'transform':
          if (!disabledTransform.includes(name) && Array.isArray(val)) {
            applyTransform(
              currentContext.ctx,
              parseTransform(val, currentContext)
            );
          }
          break;
...
export const parsePercentage = (key, val, layout) => {
  ...
  if (!Number.isNaN(percentage)) {
    return (
      (layout[yAttrs.includes(key) ? 'height' : 'width'] * percentage) / 100
    );
  }
  return val;
};
...
export function parseTransform(transform, context) {
  ...
  return transform
    .map((item) => {
      ...
      const name = ~index
        ? originName.slice(0, originName.length - 1)
        : originName;
      if (!~index) {
        index = 0;
      }
...
const getAngle = (val) => {
  return typeof val === 'number'
    ? val
    : (Number.parseFloat(val) * Math.PI) / 180;
};

function restoreOrigin(ctx, [ox = 0, oy = 0]) {
  const matrix = ctx.getTransform();

  return [ox - matrix.e / matrix.a, oy - matrix.f / matrix.d];
}
  ...
  const touch = (e) => {
    ...
    if (props && !props.disabled) {
      ...
      if (e.type === 'touchend') {
        ...
        const xy = getEventXY(e);
        if (xy.x === target.touch.x && xy.y === target.touch.y) {
...
export const calculateAspectRatioFit = ({
  ...
  return {
    ...
    destY,
    destWidth:
      destWidth || (align === 'none' ? containerWidth : contentWidth * scale),
    destHeight:
      destHeight || (align === 'none' ? containerHeight : contentHeight * scale)
  };
};
