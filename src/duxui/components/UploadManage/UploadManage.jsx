...
const Item = ({ item, uploading, isSelect, select, onSelect }) => {
  ...
  return (
    <View onClick={click} className='file-upload__item'>
      {select && (
        <View className='file-upload__item__select'>
      ...
      <View className='file-upload__item__info'>
        ...
        {uploading ? (
          <View className='file-upload__item__uploading'>
            <View
              className='file-upload__item__uploading__child'
...
const UploadList = () => {
  ...
  return (
    <>
      {list.map((item) => (
        <Item key={item.path} item={item} uploading />
      ))}
    </>
  );
};
    ...
    () => ({
      select: (_type, _max = 9) => {
        ...
        const uploadTypes = _type
          ? uploadManage.getTypes(_type).reduce(
              (prev, current) => {
                let media = 'file';
                if (current.type === 'ext') {
      ...
      <PullView onClose={close}>
        <View className='file-upload'>
          ...
          <Input placeholder='搜索' />
            ...
            {max > 1 && (
              <Text className='file-upload__header__submit' onClick={submit}>
                确定
              </Text>
            )}
          </View>
