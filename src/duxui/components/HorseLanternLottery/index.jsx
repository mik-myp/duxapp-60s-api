...
export const HorseLanternLottery = ({
  ...
  return (
    <Layout style={{ ...style, ...gapStyle }} onLayout={setLayout} {...props}>
      {!!layout.width && (
        <>
          <Row style={gapStyle}>
            {Array(column)
          ...
          <Row style={gapStyle}>
                ...
                .map((_i, index) => {
                  return (
                    <ItemSelf
                      key={index}
                      list={list}
                      index={row - 2 - index - 1 + column + row + column - 2}
                      Item={Item}
                      select={select}
                    />
                  );
            ...
            <Column
              ...
              style={{
                width:
                  ((layout.width - pxNum(gap) * (column - 1)) / column) *
                    (column - 2) +
                  pxNum(gap) * (column - 3)
              }}
              onClick={start}
            >
              {isValidElement(Start) ? Start : <Start />}
            </Column>
            <Column grow className='w-0' style={gapStyle}>
              {Array(row - 2)
                .fill(1)
              ...
              .map((_i, index) => {
                return (
                  <ItemSelf
                    key={index}
                    list={list}
                    index={column - index - 2 + column + row - 1}
                    Item={Item}
                    select={select}
                    grow
                    className='w-0'
