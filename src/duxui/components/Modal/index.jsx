...
const ModalContent = forwardRef(
  ({ children, overlayOpacity = 0.2, onClose }, ref) => {
    ...
    useEffect(() => {
      nextTick(() => {
        ...
        setMaskAn(
          an.current.opacity(refs.current.overlayOpacity).step().export()
        );
      });
    }, []);
    ...
    return (
      <View className='ModalView'>
        ...
        />
        <View className='inset-0 absolute' onClick={onClose} />
        <Animated.View animation={mainAn} className='ModalView__main'>
          {children}
        </Animated.View>
      </View>
    );
...
export const Modal = ({
  ...
  useEffect(() => {
    if (show) {
      const ele = (
        <ModalContent ref={ref} onClose={close} overlayOpacity={overlayOpacity}>
          {children}
        </ModalContent>
      );
      if (action.current) {
