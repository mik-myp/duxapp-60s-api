...
export const Sign = forwardRef(
  ({ color = '#333', size = 3, style, className, onChange }, ref) => {
    const data = useRef({
      touchCount: 0,
      startCount: 0
    });
    ...
    const panResponderDrawLine = useMemo(() => {
      return PanResponder.create({
        ...
        onPanResponderMove: (evt) => {
          ...
          setDrawPath(
            (old) => `${old}${old.endsWith('L') ? '' : ','}${pointX},${pointY}`
          );
        },
    ...
    return (
      <View className={className} style={style}>
        <SvgToImage ref={svgToImage} option={{ format: 'png', quality: 0.8 }}>
