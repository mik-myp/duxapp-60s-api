...
const Image = ({ style, className, src, containerLayout, onClick }) => {
  ...
  return (
    <TaroImage
      style={{
        width:
          containerLayout.width + (process.env.TARO_ENV === 'rn' ? 0 : 'px'),
        ...style
      }}
      className={className}
      src={src}
    ...
    <TaroVideo
      style={{
        width:
          containerLayout.width + (process.env.TARO_ENV === 'rn' ? 0 : 'px'),
        height:
          (containerLayout.width * 9) / 16 +
          (process.env.TARO_ENV === 'rn' ? 0 : 'px'),
        ...style
      }}
      className={className}
      controls={!!controls}
...
const Audio = ({ src, autoplay, loop }) => {
  return <TaroView className='html-audio'></TaroView>;
};
...
const View = ({ children, style = {}, className, onClick, ...props }) => {
  ...
  // 是否是纯文本组件
  const isTextComp = useMemo(
    () =>
      !isText && children?.every?.((item) => item?.props?.nodeName === 'Text'),
    [isText, children]
  );
  ...
  return isText ? (
    ...
    <TextPlatform style={textStyle} className={className}>
      {children}
    </TextPlatform>
  ) : (
    <TaroView
    ...
    >
      {children?.map?.((item, index) => {
        return typeof item === 'string' ? (
          <TextPlatform key={index} style={textStyle}>
            {item}
          </TextPlatform>
        ) : (
          item
...
const TextPlatform = ({ children, style, className, ...props }) => {
  if (!children) {
    return null;
  }
...
const Text = ({ children, style = {}, childOther, className }) => {
  // 如果子元素里面是纯文本就是组件形式渲染，否则省略当前层级然他正常渲染
  return !childOther ? (
    <TextPlatform style={style} className={className}>
      {children?.map?.((item, index) => {
        return typeof item === 'string' ? (
          <TextPlatform style={style} className={className} key={index}>
            {item}
          </TextPlatform>
        ) : (
          item
...
const Item = ({
  ...
  return (
    <Comp containerLayout={containerLayout} onClick={onClick} {...props}>
      {child?.map((item) => {
...
export function HtmlView({
  ...
  return (
    <Layout
      onLayout={layout}
      style={{
        ...(process.env.TARO_ENV === 'h5' ? { fontSize: '16px' } : {}),
        ...style
      }}
      className={className}
    >
...
const getNodes = (() => {
  ...
  'span,font,b,strong,i,em,u,sup,sub,h1,h2,h3,h4,h5,h6,font,q,s,strike,del,big,small'
    .split(',')
    .forEach((key) => {
      nodeArr[key] = 'Text';
    });
  ...
  // 样式名称转换
  const styleNameTransforms = [['textDecoration'], ['textDecorationLine']];
  // 不支持的Style属性
    ...
    HTMLParser(html, {
      ...
      chars(text) {
        ...
        if (
          ...
          node.child?.some(
            (v) =>
              !(typeof v === 'string' || v.nodeName === 'Text') || v.childOther
          )
        ) {
