...
export const Range = ({ name, value = 12, min = 1, max = 100, onChange }) => {
  return (
    <Row className='items-center gap-1'>
      ...
      <Text style={{ width: px(32) }} align='center'>
        {value}
      </Text>
    </Row>
  );
...
export const Color = ({ value, onChange, addMenu }) => {
  ...
  const el = show && (
    <Row className='justify-between items-center' grow>
      {colors.map((color) => (
        <Column
          key={color}
          ...
          style={{
            ...
            borderColor: duxappTheme.pageColor
          }}
          onClick={() => {
            onChange(color);
            setShow(false);
  ...
  return (
    <Row className='gap-1 items-center' onClick={() => setShow(true)}>
      <Column
        className='r-1 square border-w1'
        style={{
          width: px(36),
...
export const FillRadio = ({ value, onChange }) => {
  return (
    <Row items='center' className='gap-2'>
      <Column
        className='items-center justify-center square r-1'
        style={{
          width: px(46),
          backgroundColor: !value ? duxappTheme.pageColor : 'transparent'
        }}
        onClick={() => value && onChange(false)}
      >
        <Column className='square bg-primary r-1' style={{ width: px(12) }} />
      </Column>
      <Column
        className='items-center justify-center square r-1'
        style={{
      ...
      >
        <Column className='square bg-primary r-1' style={{ width: px(28) }} />
      </Column>
    </Row>
  );
};
