    ...
    return (
      <SvgToImage ref={svgToImage}>
        <Svg {...props}>
          ...
          {path && <Painting {...pathProps} onSubmit={addLayer} />}
          {vector && (
            <Vector {...vectorProps[mode]} mode={mode} onSubmit={addLayer} />
          )}
          {mode === 'text' && <TextInput {...textProps} onSubmit={addLayer} />}
        </Svg>
      </SvgToImage>
    );
  }
...
const ItemRender = ({ item, select, setSelect, onChange, Comp }) => {
  ...
  const scalePan = useRef(
    new Animated.Value(transform.scale ?? 1, { useNativeDriver: true })
  ).current;

  const rotatePan = useRef(
    new Animated.Value(transform.rotate ?? 0, { useNativeDriver: true })
  ).current;
...
const ItemAction = ({
  ...
  const scaleEvent = useRef(
    PanResponder.create({
      ...
      onPanResponderMove: (e, gestureState) => {
        const centerX =
          (transform.translateX || 0) + layout.x + layout.width / 2;
        const centerY =
          (transform.translateY || 0) + layout.y + layout.height / 2;
        ...
        scalePan.setValue(
          Math.max(
            0.2,
            ((layout.width / 2) * scale + projection) / (layout.width / 2)
          )
        );
      },
      onPanResponderRelease: () => {
