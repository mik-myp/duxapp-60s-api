...
export const Menu = ({ round, children, className, style, ...props }) => {
  ...
  return (
    <>
      <Layout
        className={classNames('Menu flex-row items-center', className)}
        style={style}
        {...props}
        reloadKey={reloadKey}
        onLayout={onLayout}
      >
        <menuContext.Provider
          value={{ itemClick, showIndex: show, onIndex, onSetClick }}
        >
          {children}
        </menuContext.Provider>
      </Layout>
...
export const MenuItem = (() => {
  ...
  return forwardRef(
    (
      ...
      const { itemClick, showIndex, onIndex, onSetClick, pull } =
      ...
      const label = children
        ? title
        : options?.find((v) => v.value === value)?.name || title;
      ...
      return (
        <Row
          ...
          onClick={click}
        >
          <Text type={type} numberOfLines={1}>
            {label}
          </Text>
          {renderIcon || (
            <DuxuiIcon
      ...
      <Absolute>
        ...
        <Animated.View
          ...
          className={classNames(
            'Menu__content absolute',
            round && 'Menu__content--round'
          )}
          style={{
        ...
        >
          {option.current.children || (
            <Grid column={option.current.column} gap={36}>
              {option.current.options.map((item) => (
                <Text
                  align={option.current.align}
                  key={item.value}
                  type={
                    item.value === option.current.value ? 'primary' : void 0
                  }
        ...
        <Row
          className={classNames('Menu absolute', className)}
          items='center'
          style={{
            top: layout.top,
            left: layout.left,
          ...
          <formContext.Provider value={form}>
            <menuContext.Provider
              value={{
                itemClick,
                showIndex: ~show
                  ? pullIndexs[indexs.current.show.indexOf(show)]
                  : show,
                onIndex: pullIndex,
                pull: true
              }}
