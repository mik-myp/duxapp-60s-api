      ...
      (layout) => {
        (async () => {
          ...
          if (clickSize.current.top + layout.height > height) {
            newposition.top =
              height - layout.height - (height - y - viewHeight);
          }
            ...
            >
              {renderContent ||
                menuList?.map((item, index) => {
                  if (item.type === 'line') {
                    return (
                      <View key={index} className='DropDown__item--line ' />
                    );
                  }
                  return (
                    <View
                      className={`DropDown__item${select === index ? ' DropDown__item--select' : ''}`}
                      key={item[rangeKey] || item}
                      onClick={submit.bind(null, item, index)}
                    >
                      <Text className='DropDown__item__text'>
                        {item[rangeKey] || item}
                      </Text>
                      {select === index && (
                        <DuxuiIcon
