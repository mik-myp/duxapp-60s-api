...
export const PickerView = ({
  ...
  const range = useMemo(() => {
    return (
      children?.map?.((column) => {
        return column.props.children?.map(
          (item, index) => item.props.value ?? index
        );
      }) || []
    );
  }, [children]);
  ...
  const select = useMemo(() => {
    let indexs = range.map((item, index) =>
      item.findIndex((v) => v === value[index])
    );
    /**
  ...
  return (
    <TaroPickerView
      ...
      value={select}
      {...props}
      className={classNames('PickerView', grow && 'flex-grow', className)}
    >
      {children}
    </TaroPickerView>
  );
...
export const PickerViewColumnItem = ({ value, children }) => {
  return children;
};
