  ...
  const DatePicker_ = (props) => {
    ...
    useEffect(() => {
      if (propValue) {
        if (
          propValue !==
          (timestamp ? refs.date.getTime() : formatDateToString(refs.date))
        ) {
    ...
    const getDate = () => date;
    const getMinDate = () =>
      minDate ? parseDateString(minDate, format) : defaultMinDate;
    const getMaxDate = () =>
      maxDate ? parseDateString(maxDate, format) : defaultMaxDate;
      ...
      formatFields.forEach((field) => {
        switch (field) {
          ...
          case 'second': {
            ...
            if (
              formatFields.includes('hour') &&
              formatFields.includes('minute')
            ) {
              if (
                currentDate.getFullYear() === minDate.getFullYear() &&
                currentDate.getMonth() === minDate.getMonth() &&
