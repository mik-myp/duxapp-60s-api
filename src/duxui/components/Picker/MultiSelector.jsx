...
export const MultiSelectorPicker = ({
  ...
  return (
    <PickerView range={range} rangeKey={nameKey} {...props}>
      {range.map((item, index) => (
        <PickerViewColumn key={index}>
          {item.map((_item) => {
            ...
            return (
              <PickerViewColumnItem key={value} value={value}>
 ...
 * @returns
 */
MultiSelectorPicker.getShowText = (
  value,
  { range = [], nameKey = 'name', valueKey = 'value' } = {}
) => {
  if (!value?.length) {
    return '';
  }
  ...
  return range.map((item, index) => {
    const _item = item.find(
      (v) => (isObject ? v[valueKey] : v) === value[index]
    );
    if (_item) {
