...
export const Tab = ({
  ...
  const [select, setSelect] = useState(
    value ?? defaultValue ?? list?.[0]?.paneKey
  );
  ...
  const tabs = () => (
    <Scroll scroll={scroll} style={expand && scroll ? undefined : tabStyle}>
      {list.map((item, index) => (
        <TabItemRender
          type={type}
  ...
  return (
    <View
      className={classNames('Tab', className, justify && 'flex-grow')}
      style={style}
      {...props}
    >
      {oneHidden && list.length < 2 ? null : expand && scroll ? (
        <Layout
          className='flex-row'
          style={tabStyle}
          onLayout={setLayout}
          reloadKey={showExpand ? 1 : 0}
        >
          <Row grow>
          ...
          <BoxShadow
            className='Tab__expand'
            x={-10}
            onClick={() => setShowExpand(true)}
          >
            <DuxuiIcon name='more-horizontal' size={48} />
          </BoxShadow>
          {showExpand && !!layout.width && (
            <Absolute>
              <Column
                className='Tab__expand__mask-top absolute left-0 right-0 top-0'
                style={{ height: layout.top }}
              />
              <Column
                className='Tab__expand__mask absolute left-0 right-0 bottom-0'
              ...
              <Space
                row
                wrap
                className='Tab__expand__content absolute left-0 right-0'
                style={{ top: layout.top }}
              >
                {list.map((item, index) => (
                  <TabItemRender
      ...
      {tabPane.some((v) => typeof v.el !== 'undefined') && (
        <RenderContent
          swiper={swiper}
...
const RenderContent = ({
  ...
  return (
    <View className='Tab__con'>
      {tabPane.map((item) => (
        <View
          key={item.paneKey}
          className={classNames(
            'Tab__con__item',
            item.paneKey === select && 'Tab__con__item--select'
          )}
        >
          {(!lazyload || shows.current.includes(item.paneKey)) && item.el}
        </View>
    ...
    return (
      ...
      >
        <BadgeText badgeProps={badgeProps} outside>
          <Text style={text} color={select ? 4 : 1} size={4}>
            {title}
          </Text>
        </BadgeText>
      </Column>
  ...
  return (
    ...
    >
      <BadgeText badgeProps={badgeProps}>
        <Text
          style={text}
          type={select ? 'primary' : void 0}
          color={select ? void 0 : 2}
          bold
          size={4}
        >
          {title}
...
const BadgeText = ({ children, badgeProps, outside }) => {
  ...
  return (
    <Badge outside={outside} {...badgeProps}>
      {children}
    </Badge>
  );
};

const Scroll = ({ scroll, style, children }) => {
  if (scroll) {
    return (
      <ScrollView.Horizontal style={style} className='Tab__row'>
        {children}
      </ScrollView.Horizontal>
    );
  }
  return (
    <Row className='Tab__row' items='center' justify='around' style={style}>
      {children}
    </Row>
  );
};
