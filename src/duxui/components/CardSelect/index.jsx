...
export const CardSelectGroup = ({
  ...
  const isCheck = useCallback(
    (val) => (checkbox ? value?.includes?.(val) : value === val),
    [checkbox, value]
  );
  ...
  return (
    <context.Provider value={{ isCheck, check, checkedProps }}>
...
export const CardSelect = ({
  ...
  const [viewStyle, textStyle] = useMemo(() => {
    ...
    const _borderColor = select
      ? checkedProps?.borderColor || checkedProps?.color || borderColor
      : borderColor;
    const _plain =
      select && checkedProps?.plain !== undefined
        ? checkedProps?.plain
        : select
          ? false
          : plain;
    const _border =
      select && checkedProps?.border !== undefined
        ? checkedProps?.border
        : border;
  ...
  return (
    ...
    >
      {Children.map(children, (child) =>
        cloneElement(child, { style: { ...textStyle, ...child.props.style } })
      )}
    </View>
  );
};
