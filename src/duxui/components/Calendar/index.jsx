...
export const Calendar = ({
  ...
  const values = useMemo(
    () =>
      (checkbox ? value : value ? [value] : []).map((item) =>
  ...
  const [disabledDateCache, enanledDateCache] = useMemo(() => {
    return [deepData.disabledDate || [], deepData.enabledDate || []].map(
      (item) => {
        return item.reduce((prev, current) => {
          if (Array.isArray(current) && current.length === 2) {
            prev.push(
              current
                .map((v) => strFormatToDate('yyyy-MM-dd', v).getTime())
                .sort((a, b) => a - b)
            );
  ...
  const isDisabled = useCallback(
    (dayTime) => {
      return (
        disabledDateCache.some(
          ([start, end]) => dayTime >= start && dayTime <= end
        ) ||
        (enabledDate &&
          !enanledDateCache.some(
            ([start, end]) => dayTime >= start && dayTime <= end
          ))
      );
    },
      ...
      date.forEach((item) => {
        if (Array.isArray(item) && item.length === 2) {
          ...
          for (let i = dates[0]; i <= dates[1]; i += 24 * 60 * 60 * 1000) {
            ...
            prev[day] = {
              config: prev[day]?.config
                ? { ...prev[day]?.config, ...config }
                : config,
              customType:
                dates[0] === dates[1]
                  ? 'select'
          ...
          prev[item] = {
            config: prev[item]?.config
              ? { ...prev[item]?.config, ...config }
              : config,
    ...
    (config) => {
      ...
      const getRes = (render) => {
        ...
        if (
          ...
          render?.__proto__ === Object.prototype
        ) {
          return render;
        }
        return render({
          ...config,
          date: day,
          customType: customDateCache[day]?.customType
        });
      };
      const _config = {};
      if (userConfig) {
  ...
  const list = useMemo(() => {
    ...
    // 算出上个月的最大天数
    const lastMaxDay = getMaxDay(
      ...dateToStr('yyyy-MM', getMouth(month, -1))
        .split('-')
        .map((v) => +v)
    );
    ...
    const beyond = (() => {
      if (
        max &&
        strFormatToDate('yyyy-MM-dd', firstDay) >
          strFormatToDate('yyyy-MM-dd', max)
      ) {
        return true;
      }
      if (
        min &&
        strFormatToDate('yyyy-MM-dd', `${month}-${maxDay}`) <
          strFormatToDate('yyyy-MM-dd', min)
      ) {
        return true;
      }
      return false;
    ...
    for (let i = 0; i < maxDay; i++) {
      ...
      if (scopeStart) {
        const isSelect =
          +scopeDates[2] === i + 1 &&
          scopeDates.slice(0, 2).join('-') === month;
        if (isSelect) {
          config.select = true;
  ...
  const click = useCallback(
    ({ text, disable }) => {
      ...
      if (mode === 'day') {
        ...
        const dates = val.map((v) =>
          strFormatToDate('yyyy-MM-dd', v).getTime()
        );
        for (let i = dates[0]; i <= dates[1]; i += 24 * 60 * 60 * 1000) {
          if (isDisabled(i)) {
            return toast('范围不可选');
        ...
        if (!scopeStart) {
          if (checkbox) {
            ...
            const i = value.findIndex((v) =>
              Calendar.checkDatesOverlap([day, day], v)
            );
            if (~i) {
              const _value = [...value];
              _value.splice(i, 1);
          ...
          if (
            strFormatToDate('yyyy-MM-dd', val[0]) >
            strFormatToDate('yyyy-MM-dd', val[1])
          ) {
            val = [day, scopeStart];
          }
          const dates = val.map((v) =>
  ...
  return (
    ...
    >
      {!onlyCurrentWeek && (
        <View className='Calendar__head' style={navStyle}>
          <DuxuiIcon
            name='direction_left'
            className='Calendar__head__icon'
            onClick={prev}
          />
      ...
      {list.map((week, index) => {
        ...
        return (
          <View
            ...
            key={index}
            style={!index ? headStyle : undefined}
          >
  ...
  return (
    <View
      className={classNames(
        'Calendar__row__item',
        header && 'Calendar__row__item--head'
      )}
      onClick={click}
    >
      {customType && (
      ...
      {(renderTop || renderBottom) && (
        <View className='Calendar__row__item__other'>
          <View className='Calendar__row__item__other__item'>{renderTop}</View>
          <View className='Calendar__row__item__other__item'>
            {renderBottom}
          </View>
        </View>
      )}
...
Calendar.getWeekScopeForDay = (day) => {
  ...
  const before = weeks.indexOf(
    dateToStr('W', strFormatToDate('yyyy-MM-dd', day))
  );
  const after = 7 - before - 1;
  return [
    dateToStr('yyyy-MM-dd', dateAdd('d', -before, date)),
    dateToStr('yyyy-MM-dd', dateAdd('d', after, date))
  ];
};
...
Calendar.checkDatesOverlap = (range1, range2) => {
  const [a, b, c, d] = [...range1, ...range2].map((v) => dayjs(v).unix());
