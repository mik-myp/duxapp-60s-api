...
export const ElevatorSearch = ({
  ...
  return (
    <View className={classNames('Elevator__search', className)} {...props}>
      <Image src={searchIcon} className='Elevator__search__icon' />
      <Input
        className='Elevator__search__input'
...
export const Elevator = ({
  ...
  const resultList = useMemo(() => {
    ...
    return list.reduce((prev, current) => {
      const children = current.children.filter((item) =>
      ...
      <View className={`Elevator ${className}`} style={style} {...props}>
        ...
        <ScrollView scrollTop={scrollTop}>
          ...
          {resultList.map((group, groupIndex) => (
            <Layout
              className='Elevator__group'
              key={group.index}
              onLayout={(e) => layout(e, groupIndex + 1)}
            >
              {showNav && (
                <View className='Elevator__group__name'>{group.name}</View>
              )}
              {group.children.map((item) => {
                return (
                  <View
                    className='Elevator__group__item'
                    key={item.name}
        ...
        {showNav && (
          <View className='Elevator__nav' onTouchMove={move}>
            <Layout
