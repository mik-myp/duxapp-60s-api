...
const Row = ({
  ...
  return (
    <View
      {...rootProps}
      className={`Step__item--row ${className || ''}`}
      style={style}
    >
      {!!startSize && (
        <View className='Step__item__start' style={{ height: px(startSize) }}>
      ...
      <View className='Step__item__center--row'>
        ...
        <View className='Step__item__point z-1'>
          {renderPoint ? (
            ...
            <View
              className='Step__item__point__point'
              style={
                item?.pointColor ? { backgroundColor: item.pointColor } : {}
              }
            />
          )}
        </View>
...
const Column = ({
  ...
  return (
    <View
      {...rootProps}
      className={`Step__item--column ${className || ''}`}
      style={style}
    >
      {!!startSize && (
        <View className='Step__item__start' style={{ width: px(startSize) }}>
          {renderStart?.({ item, index })}
        </View>
      )}
      ...
      <View className='Step__item__end--column'>
        {renderEnd?.({ item, index })}
      </View>
    </View>
  );
};
...
const RowContainer = ({ row, children }) => {
  if (row) {
    return <ScrollView.Horizontal>{children}</ScrollView.Horizontal>;
  }
  return children;
};
