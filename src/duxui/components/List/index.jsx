...
export const createList = (usePageData) => {
  const List = forwardRef(function List_(
    ...
    const [list, action] = usePageData(
      ...
      { field: listField, listCallback, cache, ...option }
    );
    ...
    const RenderItem = useMemo(() => {
      const Item_ = ({
        ...
        return (
          <ListSelect.Item item={item} index={index} id={id}>
            <Item
              item={item}
              id={type ? undefined : id}
              index={index}
              {...otherProps}
              {...refs.current}
            />
    ...
    const loadMore = ((page && !action.refresh && list.length > 5) ||
      (action.loadEnd && !emptyStatus)) && (
      <ListLoading
        loading={action.loading}
        text={
          action.loading ? '加载中' : action.loadEnd ? '没有更多了' : '上拉加载'
        }
      />
    );
        ...
        {process.env.TARO_ENV === 'rn' ? (
          <FlatList
            ...
            ListEmptyComponent={
              !action.loading &&
              (renderEmpty || (
                <Empty onEmptyClick={onEmptyClick} emptyTitle={emptyTitle} />
              ))
            }
            ListFooterComponent={
              <>
...
const Empty = ({ onEmptyClick, emptyTitle }) => {
  return (
    <View className='list-empty' onClick={onEmptyClick}>
