...
export const ListSelect = ({ children }) => {
  ...
  return (
    <context.Provider value={{ type, choice, isCheck, submit }}>
      {children}
    </context.Provider>
  );
};

const ListSelectItem = ({ id, item, children }) => {
  const { type, choice, isCheck } = useContext();
  if (!type) {
    return children;
  }
  return (
    <View className='ListSelectItem' id={id}>
      <View
        className={`ListSelectItem__check${type === 'radio' ? ' ListSelectItem__check--radio' : ''}`}
        onClick={() => choice(item)}
      >
        {isCheck(item) && (
          <View
            className={`ListSelectItem__check__child${type === 'radio' ? ' ListSelectItem__check__child--radio' : ''}`}
          />
        )}
      </View>
      <View className='ListSelectItem__content'>{children}</View>
    </View>
  );
};

const ListSelectSubmit = ({ children }) => {
  const { type, submit } = useContext();
  if (!type) {
    return null;
  }
  return (
    <View onClick={submit} className='ListSelectSubmit'>
