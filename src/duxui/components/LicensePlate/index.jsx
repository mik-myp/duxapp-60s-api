...
export const LicensePlateContext = /*@__PURE__*/ createContext([
  { value: '', length: 7 },
  noop
]);

export const LicensePlateKeyboard = ({
  onInput,
  ...
  return (
    <>
      ...
      {keys.length === 1 && (
        <City province={keys[0]} onKey={onKey} onBackspace={onBackspace} />
      )}
...
export const LicensePlate = ({ onChange, ...props }) => {
  ...
  return (
    <LicensePlateContext.Provider value={state}>
      ...
      {show && (
        <PullView ref={pullView} masking={false} onClose={() => setShow(false)}>
          <BoxShadow
            className='p-3 bg-page gap-3 rt-3'
            style={{ backgroundColor: duxappTheme.pageColor }}
          >
            <LicensePlateContext.Provider value={state}>
              <Row justify='end' items='center' self='stretch'>
                <Text type='primary' onClick={() => pullView.current.close()}>
                  关闭
                </Text>
              </Row>
