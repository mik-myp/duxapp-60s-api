...
export const ImageGroup = ({ children }) => {
  ...
  return (
    <context.Provider value={{ addImage, removeImage, preview, group: true }}>
      {children}
    </context.Provider>
  );
};
      ...
      return (
        <View
          className={classNames(
            'Image--img-square__root Image',
            'Image--' + radiusType,
            className
          )}
          style={
            typeof square === 'number' ? { width: px(square), ...style } : style
          }
        >
          <TaroImage
            className={classNames(
              'Image--img-square',
              process.env.TARO_ENV !== 'weapp' && 'Image--img-square--height'
            )}
            mode='aspectFill'
            src={src}
