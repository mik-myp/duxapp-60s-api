...
export const FormItem = ({
  ...
  const check = useCallback(() => {
    ...
    return new Promise((resolve, reject) => {
      schema.validate(
        { [field]: fields ? refs.form.values : refs.value },
        (errors) => {
          if (!errors) {
            resolve();
          } else {
  ...
  const horizontal =
    (direction || form.direction) === 'horizontal' &&
    !vertical &&
    (!form.vertical || vertical === false);
    ...
    (val) => {
      ...
      if (fields) {
        ...
        if (
          val &&
          typeof val === 'object' &&
          typeof val.detail?.value !== 'undefined'
        ) {
  ...
  const cloneForm = (item) => {
    ...
    return cloneElement(item, {
      ...
      [triggerPropName]:
        item[triggerPropName] ?? (fields ? form.values : value),
    ...
    return (
      <Column
        ...
        className={classNames(
          'FormItem',
          !form.itemPadding && 'FormItem--no-padding',
          className
        )}
      >
        {child}
  ...
  const _label = (
    <Text
      {..._labelProps}
      className={classNames(
        horizontal && 'FormItem__label',
        _labelProps.className
      )}
      style={_labelProps.style}
    >
      {label}
      {required && <Text className='FormItem__label__required'>*</Text>}
      {!!subLabel && (
        <Text size={1} color={3} bold={false}>
          {' '}
          {subLabel}
        </Text>
      )}
  ...
  return (
    ...
    >
      <Space
        ...
        style={_containerProps.style}
        {..._containerProps}
      >
        {renderLabelRight ? (
          <Space row justify='between'>
            {_label}
            {renderLabelRight}
          </Space>
        ) : (
