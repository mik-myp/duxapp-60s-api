...
export const InputNumber = ({
  ...
  const stepPrecision = step.toString().includes('.')
  ...
  const inputChange = (inputValue) => {
    ...
    // 修正为步进值的倍数
    const adjustedValue = fixPrecision(
  ...
  return (
    <Row
      className={classNames(`r-1 border-w1 border-${type}`, className)}
      {...props}
    >
      <Column
        className={classNames(
          `InputNumber__btn bg-${type}`,
          (disabled || val <= min) && 'InputNumber__btn--disabled'
        )}
        justify='center'
        onClick={() => click(0)}
      >
      ...
      <Column
        className={classNames(
          `InputNumber__btn bg-${type}`,
          (disabled || val >= max) && 'InputNumber__btn--disabled'
        )}
        justify='center'
        onClick={() => click(1)}
      >
