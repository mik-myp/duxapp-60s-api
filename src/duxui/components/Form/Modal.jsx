...
export const ModalForm = ({
  ...
  const child = isValidElement(children) ? (
    ...
    >
      ...
      <Text color={3} size={5}>
        <DuxuiIcon name='direction_right' />
      </Text>
    </Row>
  );
  ...
  return (
    <>
      {child}
      {show && (
        <PullView ref={pull} onClose={() => setShow(false)} side={side}>
          <context.Provider value={{ reset, submit }}>
            <Column
              className={classNames(
                'ModalForm',
                ['left', 'right'].includes(side)
                  ? 'ModalForm--full h-full'
                  : 'ModalForm--vertical'
              )}
            >
              {!!title && (
              ...
              {showButton && !autoSubmit && (
                <>
                  ...
                  <Space row className='ModalForm__btns'>
                    ...
                    <Submit type='primary' size='l' className='flex-grow'>
                      提交
                    </Submit>
                  </Space>
                </>
...
const Submit = ({ children, value, ...props }) => {
  ...
  return (
    <Button {...props} onClick={() => form.submit(value)}>
      {children}
    </Button>
  );
};
...
const Reset = ({ children, mode, ...props }) => {
  ...
  return (
    <Button {...props} onClick={() => form.reset(mode)}>
      {children}
    </Button>
  );
};
...
export const ModalForms = ({
  ...
  return (
    <>
      {child}
      {show && (
        <PullView onClose={() => setShow(false)} side={side}>
