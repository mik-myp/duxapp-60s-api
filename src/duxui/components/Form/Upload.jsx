...
export const UploadImages = ({
  ...
  const add = useCallback(async () => {
    ...
    try {
      ...
      const urls = await upload(type, {
        count: max - (value?.length || 0),
        sizeType: ['compressed'],
        ...option
      })
        .start(() => {
  ...
  const itemClass = classNames(
    'UIUplod__item',
    isOne && 'UIUplod__item--one',
    isOne && props.className
  );
  ...
  return (
    <Grid column={column} square gap={24} {...props}>
      {content}
    </Grid>
  );
};

const ItemView = ({ src, value }) => {
  ...
  if (isImage(src)) {
    ...
    return (
      <Column className='UIUplod__item__image w-full h-full'>
        <Video
          ...
          objectFit='cover'
        />
        <Column className='inset-0 absolute' onClick={preview} />
      </Column>
    );
  }
};
...
const isImage = (url) =>
  mediaImage.some((v) => url.split('?')[0].toLowerCase().endsWith(v));
