...
const AudioRecorder = ({
  ...
  const add = useCallback(async () => {
    try {
      ...
      const urls = await upload([file], option)
        .start(() => {
          setProgress(0);
        })
    ...
    ...value?.map((item, index) => {
      return (
        ...
        >
          <ItemPlay src={item} />
          {!disabled && (
            <Column className='UIUplod__item__icon'>
              <DuxuiIcon
                name='close'
                color='red'
                size={36}
      ...
      >
        {~progress ? (
          ...
          <>
            ...
            >
              <Column
                style={{ width: px(tip ? 18 : 28) }}
                className='square r-1 bg-danger'
              />
            </Column>
            {!!tip && (
              <Text color={2} size={2}>
...
const RecorderOne = ({ onChange, value, ...props }) => {
  return (
    <AudioRecorder
      max={1}
...
const AddAudio = ({ onSubmit, onClose }) => {
  ...
  return (
    <PullView ref={pullView} onClose={onClose}>
        ...
        {result.path ? (
          <>
            ...
            {play ? (
              ...
              <Text align='center' size={1} type='primary'>
                播放
              </Text>
            )}
          </>
          ...
          <>
            <Column
              ...
              className={classNames(
                'square r-max mt-3',
                status ? 'bg-danger' : 'border-w1 border-danger'
              )}
              onClick={start}
            >
              <Column
            ...
            {status ? (
              ...
              <Text align='center' size={1} type='danger'>
                点击开始录制
              </Text>
            )}
          </>
...
const Timer = (props) => {
  ...
  return (
    <Text align='center' size={1} type='danger' {...props}>
      {[(time / 60) | 0, time % 60]
