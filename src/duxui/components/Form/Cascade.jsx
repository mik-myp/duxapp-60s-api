...
export const Cascade = ({
  ...
  const defaultSelect = useMemo(() => {
    ...
    if (
      (isRadio && typeof val !== 'undefined') ||
      (!isRadio && typeof val?.[0] !== 'undefined')
    ) {
    ...
    return select.reduce(
      (prev, _select, index) => {
        ...
        if (indexs[indexs.length - 1] === -1) {
          ...
          const _val =
            recursionGetValue(indexs, list, childrenKey)?.[childrenKey] || [];
          prev.push(_val);
        }
        return prev;
      },
  ...
  const labelClick = useCallback(
    async (_level, index, item) => {
      if (disabled) return;
      // 没有数据需要获取数据
      select[_level] = index;
      setSelect([...select]);
      if (getData) {
        const current = recursionGetValue(
          select.slice(0, _level + 1),
          list,
          childrenKey
        );
    ...
    (item) => {
      ...
      if (isRadio) {
        ...
        if (anyLevel) {
          ...
          const keys = selectList[selectList.length - 1].map(
            (v) => v[valueKey]
          );
  ...
  return (
    <View className={classNames('Cascade', className)} style={style} {...props}>
      <Render
        list={selectList}
        select={select}
        isRadio={isRadio}
...
const DefaultRender = ({
  ...
  return (
    <>
      {lefts.map((group, groupIndex) => (
        <View className='Cascade__left' key={groupIndex}>
          <ScrollView>
            {group.map((item, index) => {
              ...
              return (
                ...
                >
                  ...
                  {!isRadio &&
                    groupIndex === 0 &&
                    checkNumbers[index] > 0 &&
                    !anyLevel && (
                      <Text className='text-s1 text-primary'>
                        已选 {checkNumbers[index]}
                      </Text>
                    )}
                </View>
              );
          ...
          {right.map((item, index) => {
            ...
            return (
              <Fragment key={'item' + index}>
                {!!index && <Divider />}
                <View
                  className='Cascade__right__item'
...
const FillRender = ({
  ...
  return (
    <DividerGroup vertical>
      {list.map((group, groupIndex) => {
        const last = groupIndex == list.length - 1;
        return (
          <Column key={groupIndex} grow>
            <ScrollView>
              {group.map((item, index) => {
                const isSelect = last
                  ? isRadio
                ...
                return (
                  <Space
                    ...
                    className={classNames(
                      'Cascade-fill__item',
                      isSelect && 'Cascade-fill__item--select'
                    )}
                  >
                    <Text
