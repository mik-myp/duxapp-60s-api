...
const TabbarScreen = ({ select, index, child }) => {
  const render = useRef();
  ...
  return (
    <screenContext.Provider value={{ hover: select, index }}>
      <View
      ...
      >
        {render.current &&
          (React.isValidElement(child.Comp) ? (
            React.cloneElement(child.Comp, {
              _index: child.index,
              _key: child.itemKey
            })
          ) : (
...
const TabbarButton = ({
  ...
  return (
    <View
      className={classNames('TabBar-menu__item', Icon ? 'pv-2' : 'pv-3')}
...
const Item = ({ name, component, icon, itemKey }) => {
  return null;
};

const ItemIcon = ({ hover, name, image, imageHover, icon }) => {
  return (
    <View className='TabBar-menu__icon'>
      {icon || (
        <Image
          className='TabBar-menu__icon__image'
      ...
      {!!name && (
        <Text
          className={classNames(
            'TabBar-menu__icon__name',
            hover && 'TabBar-menu__icon__name--hover'
          )}
        >
          {name}
        </Text>
...
const TabBar = ({
      ...
      if (key === 'switch') {
        ...
        if (currentPage() !== path) {
          ...
          const index = pages.findIndex((page) => {
            const pagePath = page.route.startsWith('/')
  ...
  return (
    <tabbarContext.Provider value={{ show }}>
...
export const createTabBar = (() => {
  ...
  return () => {
    ...
    const _TabBar = ({ children, style, ...props }) => {
      return (
        <TabBar
          tabbarKey={key}
