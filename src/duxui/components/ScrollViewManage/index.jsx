...
export const ScrollViewManage = ({ defaultRefresh = false, children }) => {
  ...
  const setRefresh = useCallback(() => {
    ...
    Promise.all(
      callbacks.current.top.map(async (item) => {
        const res = item();
  ...
  const onRefresh = useCallback((callback) => {
    ...
    return {
      remove: () => {
        callbacks.current.top.splice(
          callbacks.current.top.indexOf(callback),
          1
        );
      }
    };
  ...
  const scrollToLower = useCallback(() => {
    ...
    Promise.all(
      callbacks.current.bottom.map(async (item) => {
        const res = item();
        if (res instanceof Promise) {
          await res;
        }
  ...
  const onScrollToLower = useCallback((callback) => {
    ...
    return {
      remove: () => {
        callbacks.current.bottom.splice(
          callbacks.current.bottom.indexOf(callback),
          1
        );
      }
    };
