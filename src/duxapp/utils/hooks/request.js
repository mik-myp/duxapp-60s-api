  ...
  return {
    useRequest: (option, config) => {
      ...
      const [data, setData] = useState(
        _config.current?.defaultData ??
          (config?.cache && requestCache.getCache(option)) ??
          {}
      );
      ...
      const resultAction = useCallback(
        async (res) => {
          ...
          const _data = _config.current?.field
            ? res[_config.current.field]
            : res;
          setData(_data);
    ...
    usePageData: (option, config) => {
      const requestOption = useDeepObject(
        option && typeof option === 'object' ? option : { url: option }
      );
      ...
      const getList = useCallback(() => {
        ...
        return request(_option)
          .then(async (res) => {
            ...
            state.page === 1 &&
              requestCache.setCache(state.requestOption, _list);
            state.loading = false;
            setLoading(false);
            setRefresh(false);
            return _list;
...
class RequestHookCache extends ObjectManage {
  ...
  _getKey = (option) => {
    ...
    return base64.parse(
      `${option.url}-${option.method || 'GET'}-${JSON.stringify(option.data)}-`
    );
  };
