...
const requestReact = async ({ url, data, header, timeout, ...option } = {}) => {
  ...
  const result = {
    ...
    header: Object.fromEntries(
      [...res.headers.keys()].map((key, index) => [key, headersValues[index]])
    )
  };
  ...
  return (params = {}, origin) => {
    ...
    const requestTask = new Promise(async (resolve, reject) => {
      if (middle.before?.length) {
        try {
          requestParams = await execMiddle(
            middle.before,
            requestParams,
            origin
          );
      ...
      taroRequestTask = (
        ['h5', 'rn'].includes(process.env.TARO_ENV) ? requestReact : taroRequest
      )({
  ...
  return (config) => {
    ...
    const getParams = (params) => {
      return {
        ...
        middle: {
          before: sortMiddle([
            ...(params.middle?.before || []),
            ...globalMiddle.before,
            ...middle.before
          ]),
          result: sortMiddle([
            ...(params.middle?.result || []),
            ...globalMiddle.result,
            ...middle.result
          ]),
          error: sortMiddle([
