 ...
 * @returns
 */
const execGetObject = (obj, ...params) =>
  typeof obj === 'function' ? obj(...params) : obj;
...
const chooseMedia = async (type = 'image', options = {}) => {
  ...
  const isCompressed =
    compressed || (sizeType.length === 1 && sizeType[0] === 'compressed');
  ...
  if (process.env.TARO_ENV === 'rn') {
    ...
    const option = {
      ...
      videoQuality:
        ExpoImagePicker.UIImagePickerControllerQualityType[
          isCompressed ? 'Low' : 'Medium'
        ]
    };
    let promise;
    if (source === 'photo') {
    ...
    return execMiddle(
      chooseMedia.middle?.sort((a, b) => a[1] - b[1]).map((v) => v[0]),
      files,
      options
    );
  } else {
