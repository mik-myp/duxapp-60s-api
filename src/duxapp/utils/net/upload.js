    ...
    ? (() => {
        const createFormData = (filePath, body = {}, name) => {
          ...
          const fileObj = {
            ...
            name: uri.substr(uri.lastIndexOf('/') + 1) || 'file'
          };
...
const upload = (type = 'image', option) => {
  let uploadTemp;
          ...
          allUpload.push(
            uploadFileRes.then((response) => {
              try {
                ...
                if (
                  code == (resultConfig.successCode || resultConfig.succesCode)
                ) {
                  return execGetChild(
                    option.resultField || uploadConfig.resultField,
                    response
                  );
                } else {
                  throw { code, message };
...
export const createUpload = (() => {
  ...
  return (config) => {
    ...
    return {
      ...
      uploadTempFile: (files, option) =>
        uploadTempFile(files, getOption(option)),
      middle: {
        before: (...arg) => {
