...
const recursionGetValue = (keys, data = {}, childKey, splice = false) => {
  ...
  if (keys.length === 1) {
    ...
    return recursionGetValue(
      keys.slice(1),
      childKey === undefined ? data[keys[0]] : data[keys[0]][childKey],
      childKey,
      splice
    );
  }
...
const deepEqua = (data1, data2) => {
  if (data1 === data2) {
    ...
    data1 !== null &&
    data2 !== null &&
    (typeof data1 !== 'object' || typeof data2 !== 'object')
  ) {
    return false;
...
export { recursionGetValue, deepCopy, deepEqua };
