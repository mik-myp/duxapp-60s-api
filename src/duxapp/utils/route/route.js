...
const { pages, registerPages, pagesTransfer, routes } = (() => {
  ...
  return {
    ...
    registerPages: (route, userConfig) => {
      ...
      const disablePages = Object.values(route)
        .map((item) => item.disablePages)
        .flat()
        .filter((v) => v);
      const openPages = Object.values(route)
        .map((item) => item.openPages)
        .flat()
        .filter((v) => v);
      ...
      Object.values(route)
        ...
        .map(([key, _config]) => {
          ...
          const pageConfig = {
            openPages: [...openPages, ...(userConfig.openPages || [])],
            disablePages: [...disablePages, ...(userConfig.disablePages || [])]
          };
          if (subPages) {
            // 分组
            ...
            // 普通页面
            return [key]
              .filter(() => pageFilter({ ...arg, page: key }, pageConfig))
              .map((item) => [item, arg]);
          }
        })
        .flat()
...
const pageTransfer = (path, params) => {
  const defaultKey = Object.keys(pagesTransfer.default).find((item) =>
  ...
  const startKey = Object.keys(pagesTransfer.start).find(
    (item) => item === start
  );
  if (startKey) {
    return pageTransfer(
      path.replace(startKey, pagesTransfer.start[startKey]),
      params
    );
  }
  return {
...
export { pages, routes, registerPages, pagesTransfer, pageTransfer };
