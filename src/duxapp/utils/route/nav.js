...
class Route {
  ...
  async nav(url, data = {}) {
    if (
      ...
      url !== 'back:system' &&
      new Date().getTime() - Route.time < 1000
    ) {
      return;
    }
  ...
  mini(option = {}) {
    ...
    return this.nav(
      `mini:${Object.values(params).join('|')}${path ? '|' + path : ''}`,
      query
    );
  }
  ...
  getOption(url, data) {
    ...
    if (path.startsWith('./') || path.startsWith('../')) {
      ...
      // 构建新路径
      const newParts = currentParts.slice(
        0,
        currentParts.length - backLevels - 1
      );
    ...
    switch (type) {
      ...
      case 'back':
        ...
        if (option.delta === 'home') {
          ...
          const index = Object.keys(routerPage).find(
            (key) => !routerPage[key].disable
          );
  ...
  getCurrentPages() {
    const pages =
      process.env.TARO_ENV === 'harmony_cpp' ? this.current : getCurrentPages();
  ...
  useRoute() {
    const data = useMemo(() => {
      return (
        deepCopy(this.current[this.current.length - 1]) || {
          path: '',
          params: {}
        }
      );
