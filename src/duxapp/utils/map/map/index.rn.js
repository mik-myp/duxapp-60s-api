  ...
  static openMap = (pos) => {
    const startMap = (type) => {
      switch (type) {
        case 'baidu':
          Linking.openURL(
            `baidumap://map/geocoder?location=${pos.latitude},${pos.longitude}&coord_type=bd09ll&src=com.jujiang.mingfu`
          );
          break;
        case 'amap':
          const amap = bdDecrypt(pos.latitude, pos.longitude);
          Linking.openURL(
            `${Platform.OS === 'android' ? 'androidamap' : 'iosamap'}://viewMap?sourceApplication=minfujiaju&poiname=${pos.name}&lat=${amap.lat}&lon=${amap.lon}&dev=0`
          );
          break;
      }
    };
