...
export class TopView extends Component {
  ...
  static page = (Page, props) => {
    const TopViewPage = () => {
      return <TopView {...props}>{Page}</TopView>;
    };
    return TopViewPage;
  };
...
const Position = ({ children }) => {
  return process.env.TARO_ENV === 'rn' ||
    ...
    <View className='position'>{children}</View>
  );
};

const EleItem = memo(({ item, onRemove }) => {
  ...
  if (
    ...
    (typeof item.element[0] === 'function' ||
      typeof item.element[0] === 'object')
  ) {
...
const TopViewFunc = ({
  ...
  return (
    <>
      <WeappRem />
      <View
        className={classNames('TopView', className, `duxapp-theme-${mode}`, {
      ...
      >
        ...
        <Container>
          <KeyboardAvoiding enabled={!!isForm}>{children}</KeyboardAvoiding>
