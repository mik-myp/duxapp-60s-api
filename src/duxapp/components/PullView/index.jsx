  ...
  (
    ...
    const close = useCallback(
      async (change = true) => {
        ...
        setMainAn(
          an[translate.key](pxNum(translate.value)).opacity(0).step().export()
        );
    ...
    useEffect(() => {
      nextTick(() => {
        ...
        if (side === 'center') {
          ...
          setMainAn(
            ans.current.main.translate(0, 0).opacity(1).step().export()
          );
        }
        setMaskAn(
          ans.current.mask.opacity(refs.current.overlayOpacity).step().export()
        );
      });
    }, [duration, side]);
    ...
    return (
      <Absolute group={group}>
        {masking && (
          <Animated.View animation={maskAn} className='PullView'>
            <View
              className='PullView__other'
              onClick={() => {
                if (mask) {
                  return;
                }
