...
export const View = ({ animation, style = {}, ...props }) => {
  ...
  const getDefaultValue = useCallback((type) => {
    const val =
      type in lastStyle.current
        ? lastStyle.current[type]
        : transformProps.includes(type)
          ? lastStyle.current.transform?.find(
  ...
  const getValue = useCallback((type, value) => {
    if (
      (type.startsWith('rotate') || type.startsWith('skew')) &&
      typeof value === 'number'
    ) {
      return `${value}deg`;
    }
  ...
  const result = useMemo(() => {
    ...
    const types = [
      ...new Set(animation.map((group) => Object.keys(group.action)).flat())
    ];
    ...
    const action = types.reduce((prev, type) => {
      ...
      animation.forEach((group, index) => {
        ...
        outputRange.push(
          getValue(
            type,
            group.action[type] ?? outputRange[outputRange.length - 1]
          )
        );
      });
      return prev;
    ...
    Object.keys(action).forEach((type) => {
      ...
      action[type].addListener((e) => {
        ...
        if (!transformProps.includes(type)) {
          ...
          let item = lastStyle.current.transform.find(
            (v) => typeof v[type] !== 'undefined'
          );
          if (!item) {
            item = {
      ...
      if (transformProps.includes(type)) {
        ...
        const i = action.transform.findIndex(
          (v) => typeof v[type] !== 'undefined'
        );
        if (~i) {
    ...
    action.transformOrigin = value.interpolate({
      ...
      outputRange: [
        ...animation.map((v) => v.option.transformOrigin),
        animation[animation.length - 1].option.transformOrigin
      ],
      easing: Easing.step1
    });
    ...
    return {
      ...
      start: () => {
        an = Animated.sequence(
          animation.map((group, index) => {
            return Animated.timing(value, {
              toValue: index + 1,
              duration: group.option.duration,
              easing: (
                easings[group.option.timingFunction] || easings.linear
              )(),
...
const isNativeDriver = (arr) =>
  Object.keys(arr).every(
    (item) => transformProps.includes(item.type) || item.type === 'opacity'
  );
