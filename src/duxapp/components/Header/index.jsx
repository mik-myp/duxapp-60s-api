...
export const Header = ({
  ...
  const { pages, paths } = useMemo(
    () => route.getPathPosition(Object.keys(routePages)[0]),
    []
  );
  ...
  const bgc =
    ...
    (transparent
      ...
      : style?.backgroundColor ||
        theme.header[isOld ? 'color' : 'bgColor'] ||
        '#fff');
  ...
  return (
    <headerContext.Provider value={{ ...option, onBackClick }}>
      {option.showHeader ? (
        <>
          {!show && showStatus && (
            <View
          ...
          {(show || absolute) && (
            <View
              ...
              {...props}
            >
              {(!absolute || option.rn || option.harmony) && (
                <View
                  style={{
              ...
              >
                ...
                {renderHeader ? (
                  <View
                    style={{
                      paddingRight: option.jiaonangWidth,
                      ...
                      {renderMain || (
                        <Text
                          ...
                          style={{
                            ...
                            // 文本居中判断
                            textAlign:
                              option.weapp && (option.isBack || !titleCenter)
                                ? 'left'
                                : 'center'
                          }}
                        >
      ...
      >
        {!['alipay', 'tt'].includes(process.env.TARO_ENV) && (
          <>
            {option.isBack && (
              <Image
                src={color === 'black' ? backWhite : back}
                className='Header__nav__left__icon'
              />
            )}
            {!option.isBack && option.isBackHome && (
              <Image
                src={color === 'black' ? homeWhite : home}
                className='Header__nav__left__icon'
              />
            )}
          </>
